% script run_delay_doppler_tomo_cluster
%
% CLUSTER version. Takes lists of segments and frames and farms everything
% out: the delay-Doppler product as one cluster task per frame, and the
% three 3D array products (standard, MVDR, MUSIC) through array.m. Every
% product and every segment is its own chain, so they all run in parallel.
% All outputs sit on the CSARP_standard traces.
%
% Workflow on the cluster login node:
%  1. Fill in jobs and settings below, leave check_only_en = false, run.
%     With run_chain_en = true the script submits and waits, then checks.
%  2. If you set run_chain_en = false, or the session ends before the jobs
%     finish, reload the saved chain with cluster_load_chain and run it
%     with cluster_run. Afterwards set check_only_en = true and run this
%     script again to check the outputs without resubmitting anything.
%  3. To fill in failed or missing frames, set cluster.rerun_only = true and
%     run again. Finished frames are skipped.
%
% Compiled jobs: the startup must put opr_ndh on the path (see
% STARTUP_SETUP.md), because mcc resolves every function through the path
% that startup builds. delay_doppler_task is added to the compile list
% automatically.
%
...
%
% See also: run_delay_doppler_tomo, delay_doppler_tomo,
%   delay_doppler_batch, delay_doppler_tomo_check, select_day_seg_frms
...
% frames) or frames as 'YYYYMMDD_SS_FFF'. A segment may be listed once per
% frame.
jobs = [];
jobs(1).param_fn     = 'rds_param_2024_Antarctica_GroundGHOST2.xlsx';
jobs(1).day_seg_frms = {'20250117_03', '20250118_02_004', '20250118_02_005'};
% jobs(2).param_fn     = 'rds_param_2023_Antarctica_GroundGHOST.xlsx';
% jobs(2).day_seg_frms = {'20231231_04'};
